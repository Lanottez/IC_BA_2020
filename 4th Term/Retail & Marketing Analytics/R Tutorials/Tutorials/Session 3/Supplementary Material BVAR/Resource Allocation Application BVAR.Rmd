...
```{r echo=FALSE}
data <- read_excel("resource_allocation_application.xlsx")
...
#original code - threw error at line 229 when defining a_mu_prior
#y <- data_for_var$Y  
#x <- data_for_var$Z

#new code  - fixes error from above, but throws error at line 248
y <- t(data_for_var$data$Y)  #--> commented out the data index & added $data before Y
x <- t(data_for_var$data$Z)
```
...
for (draw in 1:iter) {
  ...
  a <- post_normal(y = y, x = x, u_sigma_i, a_mu_prior, a_v_i_prior)
...
```{r}
bvar_est <- bvar(y = data_for_var$data$Y, x = data_for_var$data$Z, A = draws_a[1:9,],
