...
Q2 <- optimize(f, c(0, 800), p = 250, c = 100, s = 80, 
               d = d, maximum = T); Q2
...
f <- function(Q, p, c, s, d) mean(p*pmin(Q, d) + s*pmax((Q - d), 0) - c*Q)
Qd <- optimize(f, c(0, 2500), p = 50, c = 20, s = 10, 
               d = rnorm(10000, mean = 1000, sd = 500), maximum = T); Qd
profit_d <- 4*Qd$objective; profit_d
Qa <- optimize(f, c(0, 7000), p = 50, c = 22, s = 10, 
               d = rnorm(10000, mean = 4000, sd = 1000), maximum = T); Qa
...
qA <- 850
q1 <- 1000
profit <- mean(p*(pmin(q1, d1) + pmin(q1, d2) + pmin(q1, d3) + pmin(q1, d4) + pmin((pmax(d1 - q1, 0) 
            + pmax(d2 - q1, 0) + pmax(d3 - q1, 0) + pmax(d4 - q1, 0)), qA)) + s*(pmax((q1 - d1), 0)
            + pmax((q1 - d2), 0) + pmax((q1 - d3), 0) + pmax((q1 - d4), 0) + pmax(qA - (pmax(d1 - q1, 0)
            + pmax(d2 - q1, 0) + pmax(d3 - q1, 0) + pmax(d4 - q1, 0)), 0)) - 4*cd*q1 - ca*qA); profit