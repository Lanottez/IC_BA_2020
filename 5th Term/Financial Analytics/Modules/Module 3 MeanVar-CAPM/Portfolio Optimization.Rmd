...
---
This notebook shows how to construct the efficient frontier from mean-variance optimisation when a risk-free asset is not available. It uses the cvx package for solving convex optimisation problems. It assumes a slightly different but equivalent (why?) formulation of the problem. 

You should be able to adapt the code here to handle:

1. The problem formulation we've been using in the course.
2. The problem formulation where a risk-free asset is available.
3. The problem where the goal is to outperform or track a benchmark.
...
In this example, we solve the Markowitz portfolio problem under various constraints [@Markowitz:1952; @Roy:1952; @LoboFazelBoyd:2007].

We have $n$ assets or stocks in our portfolio and must determine the amount of money to invest in each. Let $w_i$ denote the fraction of our budget invested in asset $i = 1,\ldots,n$, and let $r_i$ be the return in the $i^{th}$ security over the period of interest. We model returns as a random vector $r \in {\mathbf R}^n$ with known mean vector ${\mathop{\bf E{}}}[r] = \mu$ and covariance matrix ${\mathop{\bf Var{}}}(r) = \Sigma$. Thus, given a portfolio $w \in {\mathbf R}^n$, the overall return is $R = r^Tw$.

Portfolio optimization involves a trade-off between the expected return ${\mathop{\bf E{}}}[R] = \mu^Tw$ and the return risk which we take to be the return variance ${\mathop{\bf Var{}}}(R) = w^T\Sigma w$. Initially, we consider only long portfolios, so our problem is
$$
\begin{array}{ll} 
...
Note how we can obtain the risk and return by _directly evaluating_ the value of the separate expressions:
...
```
An alternative is to set a lower bound on the return and minimize just the risk. To account for transaction costs, we could add a term to the objective that penalizes deviations of $w$ from the previous portfolio. These extensions and more are described in @BoydBusseti:2017. The key takeaway is that all of these convex
problems can be easily solved in `CVXR` with just a few alterations to the code above.
